...
app.get("/", async (req, res) => {
  ...
  if (sessionId) {
    ...
    if (user === null) res.sendFile(__dirname + "/login.html");
    else {
      //cook the user page
      ...
      userHtml = userHtml.replace(
        "%COLOR%",
        user.role === "admin" ? "lightcoral" : "lightblue"
      );
  ...
  try {
    ...
    if (result.rowCount === 0)
      ...
      if (successResult === true) {
        ...
        const sql =
          "update session_auth set sessionid = $1 where username = $2";
        const result = await pool.query(sql, [sessionId, req.body.user]);
        console.log(result);
        //maybe check if it succeeds..
        res.setHeader("set-cookie", [
          `SESSION_ID=${sessionId}; httponly; samesite=lax`,
        ]);
...
app.post("/register", async (req, res) => {
  ...
  if (result.rowCount === 0) {
    ...
    await pool.query(
      "insert into session_auth (username, password, role) values ($1,$2,$3)",
...
app.listen(port, () => console.log("Session Auth - Listening on " + port));
