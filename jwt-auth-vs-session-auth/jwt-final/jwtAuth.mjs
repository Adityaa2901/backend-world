...
app.post("/token", async (req, res) => {
  ...
  const result = await pool.query("select * from jwt_auth where token = $1", [
    token,
  ]);
  ...
  try {
    ...
    if (result.rowCount === 0)
      ...
      if (successResult === true) {
        ...
        pool.query("update jwt_auth set token = $1 where username = $2", [
          refreshtoken,
...
app.post("/register", async (req, res) => {
  ...
  if (result.rowCount === 0) {
    ...
    await pool.query(
      "insert into jwt_auth (username, password, role) values ($1,$2,$3)",
