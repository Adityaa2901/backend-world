...
app.post("/login", async (req, res) => {
  const sql =
    "select username from auth_method1 where username = $1 and password = $2";
  const result = await pool.query(sql, [req.body.user, req.body.password]);
...
app.listen(port, () => console.log("Auth Method 1 - Listening on " + port));
